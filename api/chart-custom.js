...
function tweakSvg(svgText) {
  ...
  // === 1) Engrosar SOLO líneas de aspectos (rojo/azul/verde) ===
  // Cubrimos stroke en atributo, en style="", y también <g ...> padre.
  const COLOR_PATTERNS = [
    // rojo
    '(?:#ff0000|#FF0000|#f00|#F00|rgb\\(\\s*255\\s*,\\s*0\\s*,\\s*0\\s*\\)|red)',
    // azul
    '(?:#0000ff|#0000FF|#00f|#00F|rgb\\(\\s*0\\s*,\\s*0\\s*,\\s*255\\s*\\)|blue)',
    // verde
    '(?:#00ff00|#00FF00|#0f0|#0F0|rgb\\(\\s*0\\s*,\\s*255\\s*,\\s*0\\s*\\)|green)',
  ];
  const COLOR_UNION = COLOR_PATTERNS.join('|');
  const SHAPES = '(?:line|path|polyline|polygon|circle|ellipse)';

  // a) Elementos con stroke="color": reemplazo/inyecto stroke-width
  out = out.replace(
    new RegExp(`(<${SHAPES}\\b[^>]*stroke="(?:${COLOR_UNION})"[^>]*?)\\s+stroke-width="[^"]+"([^>]*>)`, 'g'),
    '$1 stroke-width="4" stroke-linecap="round"$2'
  );
  out = out.replace(
    new RegExp(`(<${SHAPES}\\b[^>]*stroke="(?:${COLOR_UNION})"(?:(?!stroke-width)[^>])*?)(>)`, 'g'),
    '$1 stroke-width="4" stroke-linecap="round"$2'
  );

  // b) Elementos con style="...stroke:color...": reemplazo/inyecto stroke-width en style
  out = out.replace(
    new RegExp(`(<${SHAPES}\\b[^>]*style="[^"]*stroke:\\s*(?:${COLOR_UNION})[^"]*?stroke-width:)\\s*[^;"]+`, 'g'),
    '$1 4'
  );
  out = out.replace(
    new RegExp(`(<${SHAPES}\\b[^>]*style="[^"]*stroke:\\s*(?:${COLOR_UNION})[^"]*")(?![^>]*stroke-width)([^>]*>)`, 'g'),
    '$1; stroke-width:4; stroke-linecap:round"$2'
  );

  // c) Grupos <g ... stroke="color">: agregamos stroke-width a nivel grupo para heredar
  out = out.replace(
    new RegExp(`(<g\\b[^>]*stroke="(?:${COLOR_UNION})"[^>]*?)\\s+stroke-width="[^"]+"([^>]*>)`, 'g'),
    '$1 stroke-width="4" stroke-linecap="round"$2'
  );
  out = out.replace(
    new RegExp(`(<g\\b[^>]*stroke="(?:${COLOR_UNION})"(?:(?!stroke-width)[^>])*?)(>)`, 'g'),
    '$1 stroke-width="4" stroke-linecap="round"$2'
  );
  out = out.replace(
    new RegExp(`(<g\\b[^>]*style="[^"]*stroke:\\s*(?:${COLOR_UNION})[^"]*")(?![^>]*stroke-width)([^>]*>)`, 'g'),
    '$1; stroke-width:4; stroke-linecap:round"$2'
  );

  // === 2) Divisores blancos del aro externo ===
  out = injectWhiteDividers(out);

  return out;
}

// ——— Inyecta 12 divisores blancos en el aro externo (alineados)
function injectWhiteDividers(svgText) {
  ...
  // Radios ajustados para quedar centrados en el aro negro
  const r1 = half * 0.89;
  const r2 = half * 0.99;
  const N  = 12;
...
module.exports = async (req, res) => {
  try {
    ...
    // 3) Gráfico — pedimos SVG (aro exterior negro + íconos blancos), luego tweak
    let chartUrl = null;
    let chartError = null;
    try {
      const chartResp = await astroCall(EP_WESTERN_CHART, {
        ...
        image_type: "svg",
        chart_size: 500,
        sign_background: "#000000",   // aro negro
        sign_icon_color: "#FFFFFF",   // signos blanco
        planet_icon_color: "#000000", // (deja los símbolos de planetas en negro dentro)
    ...
    try { houses = await astroCall(EP_HOUSES, astroBase);
      house1 = houses && (houses.houses || houses).find(h => String(h.house) === "1"); } catch (_){}
