...
const ALLOWED_EXACT = new Set([
  ...
  // si tu store fijo en myshopify debe quedar:
...
function originIsAllowed(origin) {
  ...
  try {
    ...
    // por host (por si cambia http/https)
...
function applyCors(res, origin) {
  const allow = originIsAllowed(origin) ? origin : ""; // si quisieras abrir: usa "*"
  if (allow) {
    res.setHeader("Access-Control-Allow-Origin", allow);
    // Si alguna vez necesitaras cookies/credenciales:
  ...
  // Cachear el preflight 10 minutos
...
// ====== Texto ES para planets/signos + formato grados (para positions) ======
const NOMBRE_ES = {
  sun: "Sol",
  moon: "Luna",
  mercury: "Mercurio",
...
module.exports = async (req, res) => {
  try {
    ...
    // Acumulador de errores no fatales (para que el front decida)
    const errors = {};
    ...
    try {
      ...
      errors.chart = "Error al pedir el gráfico a AstrologyAPI.";
      errors.detail = detailFromError(e); // útil para tu <details> del front
    }
    ...
    try {
      ...
      errors.planets = "Error al pedir posiciones a AstrologyAPI.";
      errors.planetsDetail = detailFromError(e);
    }
    ...
    // 5) Ascendente (estimado) — casa 1 de house_cusps/tropical (si falla, se omite)
    ...
    // 7) Respuesta OK (aunque el gráfico haya fallado, devolvemos 200 con errors.chart)
    res.statusCode = 200;
    res.setHeader("Content-Type", "application/json; charset=utf-8");
    res.end(JSON.stringify({
      ...
      errors, // <- si hay errores no fatales, van acá
    }, null, 2));
  } catch (err) {
